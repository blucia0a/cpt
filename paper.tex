...
Prior work has previously shown that some limited forms of provenence help with
debugging~\cite{badapples, whylineicse, tipslicingsurvey}.  One effort tracked
code that wrote unusable values into variables~\cite{badapples}.  That work
showed that revealing the connection between an unusable value's origin and a
...
to debug six real-world programs studied previously in the debugging
...
in this work, including both atomicity and ordering violations.
...
apache1&  2.0.48 &  21285   & Atom.  &Yes  &No    & Yes   & No  \\%All values usable
...
violation bugs, last writer slices are effective and useful.  In all cases, we
found that the last writer slice revealed the provenance of the data involved
in the operation that caused the failure, so last writer slicing was effective.
We found that none of the failures we looked at were trivially debuggable.
This finding is consistent with the difficulty of debugging concurrency bugs --
the core dump does not reveal the interaction between threads that led to a
failure, only the result of that interaction that leads to the failure.  

Last writer slices were obviously useful in five of six cases.  In these cases,
...
be difficult.  For the remaining case, {\tt apache2}, last writer slices would
probably help with debugging, but it is less obvious because this bug is a
complex, multi-variable bug.  We provide more detail on {\tt apache2} in Section~\ref{sec:eval:debugging:cases}.

Our evaluation shows that bad value origin tracking~\cite{badapples} is not
likely to be helpful in five out of six cases where last writer slices is
...
deleted.  In the other five cases we looked at, the values involved in causing
the failure are usable and would not be tracked by bad value origin tracking.
...
%use of the lock before its initialization.     This revalation is of limited use as it does not rule out
...
be atomic with respect to the insert code to prevent this failure. 
