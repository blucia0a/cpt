...
Several techniques have been proposed analyze dependences and communication in
concurrent programs.  A closely related recent effort on dynamic dependence
analysis is Octet~\cite{octet}, which captures inter-thread communication
soundly and to aid in building arbitrary concurrency analyses. Octet tracks the
``locality state'' of memory locations to make dependence tracking efficient.
Octet differs from \ctraps in that it focuses on soundness, was built primarily
to optimize dependence tracking, and does not discuss direct use of dependence
information to help with debugging.  Octet's average run time overhead (26\%)
is comparable to the overhead of both \lws and \ctraps and Octet, \lws, and
\ctraps all have low enough overheads for production use in many cases.
