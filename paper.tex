...
\newcommand{\lwtfull}{Last Writer Table\xspace}
...
%\doublespacing
\authorinfo{Brandon Lucia}{Carnegie Mellon University, Department of Electrical and Computer Engineering}{blucia@ece.cmu.edu}
\authorinfo{Luis Ceze}{University of Washington, Department of Computer Science and Engineering}{luisceze@cs.washington.edu}
...
%{blucia@cmu.edu luisceze@cs.washington.edu\\{\bf In Submission --- Please Do Not Distribute}}
...
\begin{document}
 ...
 \thispagestyle{fancy}
  %For papers in which all authors are employed by the US government, un-comment the following
  %\def\ieeenotice{U.S. Government work not protected by U.S. copyright}
  %For papers in which all authors are employed by a Crown government (UK, Canada, and Australia), uncomment the following
  %\def\ieeenotice{978-1-4799-8161-8/15/\$31.00 {\copyright}2015 Crown}
  %For all other papers, uncomment the following:
...
We propose {\em \lwsfull} (\lws), a mechanism for tracking {\em data
...
\noindent {\bf Last writer slicing} ({\em LWS}): a provenance tracking mechanism
useful for concurrency debugging.  We present an \lws implementation
efficient enough for production use.

\vspace{.5ex}
\noindent {\bf Communication traps} (\ctraps): an extensible
framework for implementing concurrency analyses, built on \lws.  

\vspace{.5ex}
\noindent {\bf Efficacy evaluation}: We show that \lws is useful for debugging with
...
\vspace{.5ex}
\noindent {\bf Performance evaluation}: We show that in many cases our designs have
